...
class InstagramSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        next_page_bool = \
            data['entry_data']['ProfilePage'][0]['graphql']['user']['edge_owner_to_timeline_media']['page_info']['has_next_page']
        ...
        for i in edges:
            ...
            if video:
                date_posted = datetime.fromtimestamp(i['node']['taken_at_timestamp']).strftime("%d/%m/%Y %H:%M:%S")
                captions = ""
                if i['node']['edge_media_to_caption']:
                    for i2 in i['node']['edge_media_to_caption']['edges']:
                        captions += i2['node']['text'] + "\n"

                image_url = i['node']['display_url'].replace("\u0026", '&')
                item = {
                    'postURL': url,
